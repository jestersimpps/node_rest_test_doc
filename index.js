...
module.exports = (testData,options) => {
	...
	const endpointRoot = options.root;
	...
	describe(
		...
		() => {
			...
			testData.postObject && it(`Should create a new ${routeNameSingle}`, (done) => {
				request(api)
					.post(`${endpointRoot}/${testData.routeName}`)
					.auth('user', 'pass')
			...
			it(`Should return an array of ${testData.routeName}`, (done) => {
				request(api)
					.get(`${endpointRoot}/${testData.routeName}`)
					.auth('user', 'pass')
					.end((err, res) => {
			...
			testData.expectedObjectAfterPost && it(`Should get the new ${routeNameSingle}`, (done) => {
				request(api)
					.get(`${endpointRoot}/${testData.routeName}/${_ids[0]}`)
					.auth('user', 'pass')
					.end((err, res) => {
			...
			testData.putObject && it(`Should change a ${routeNameSingle}`, (done) => {
				request(api)
					.put(`${endpointRoot}/${testData.routeName}/${_ids[0]}`)
					.auth('user', 'pass')
			...
			testData.putObject && it(`Should remove a ${routeNameSingle}`, (done) => {
				request(api)
					.delete(`${endpointRoot}/${testData.routeName}/${_ids[0]}`)
					.auth('user', 'pass')
					.end((err, res) => {
